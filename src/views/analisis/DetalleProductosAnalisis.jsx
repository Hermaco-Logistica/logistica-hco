...
export const DetalleProductosAnalisis = ({ role, solicitudes = [] }) => {
  ...
  // Agrupar productos de todas las solicitudes
  const todosLosProductos = useMemo(() => {
    ...
    solicitudes.forEach((s) => {
      ...
      s.productos.forEach((p) => {
        const desc = (p.desc || p.descripcion || 'Producto sin descripción').trim().toUpperCase();
        const key = normalizarBusqueda(desc);
        if (!key) return;

        const cant = Number(p.cant || 1);
        const marca = p.marca?.trim() || '';
        const esPedido = p.estadoItem === 'Pedido' || p.estadoItem === 'Comprado' || s.estado === 'Pedido' || s.estado === 'Comprado';
  ...
  const productosFiltrados = useMemo(() => {
    ...
    return todosLosProductos.filter(p => 
      normalizarBusqueda(p.desc).includes(term) ||
      normalizarBusqueda(p.marcaPrincipal).includes(term)
    );
  ...
  const paginatedProductos = productosOrdenados.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
