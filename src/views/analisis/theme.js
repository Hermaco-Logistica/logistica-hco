...
export const formatearTiempoRespuesta = (diasNum) => {
  ...
  return {
    ...
    subtexto: 'Promedio creación → cotización',
...
export const evaluarEstadoGanada = (itemOEstado) => {
  ...
  if (typeof itemOEstado === 'string') {
    ...
    if (itemOEstado.itemOriginal || itemOEstado.estadoItem) {
      const estadoItem = (itemOEstado.itemOriginal?.estadoItem || itemOEstado.estadoItem || '').trim();
      const estadoSolicitud = (itemOEstado.solicitudOriginal?.estado || itemOEstado.estado || '').trim();
      
      // Si la solicitud entera fue Pedido o Comprado al 100%
      if (estadoSolicitud === 'Pedido' || estadoSolicitud === 'Comprado') {
        estado = 'Pedido';
      } else if (estadoItem === 'Pedido' || estadoItem === 'Comprado') {
        // O si específicamente este ítem fue comprado/pedido
        estado = 'Pedido';
      } else if (estadoSolicitud === 'Pedido Parcial' || estadoItem === 'Pedido Parcial') {
        // La solicitud tuvo compra parcial, pero este ítem en particular no fue pedido
        estado = 'No Comprado (Parcial)';
      } else {
        estado = estadoItem || estadoSolicitud;
      }
    } else {
  ...
  // 2. Pedido Parcial (RFQ con compra parcial, no cerrada al 100%)
